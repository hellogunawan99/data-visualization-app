...
export default function TesChart() {
  ...
  const fetchData = async (start, end) => {
    ...
    setError('');
    try {
      const response = await fetch(`/api/unscheduleUnitData?startDate=${start}&endDate=${end}`);
  ...
  const fetchMTDData = async () => {
    try {
      const response = await fetch('/api/mtdUnscheduleUnitData');
  ...
  return (
    <Layout>
      <div className="container mx-auto p-4">
        <h1 className="text-2xl font-bold mb-4">Unschedule Jigsaw</h1>
