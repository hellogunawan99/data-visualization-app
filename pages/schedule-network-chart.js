...
export default function TesChart() {
  ...
  const fetchData = async (start, end) => {
    ...
    setError('');
    try {
      const response = await fetch(`/api/schedulePortableData?startDate=${start}&endDate=${end}`);
  ...
  const fetchMTDData = async () => {
    try {
      const response = await fetch('/api/mtdSchedulePortableData');
  ...
  return (
    <Layout>
      <div className="container mx-auto p-4">
        <h1 className="text-2xl font-bold mb-4">Schedule Network</h1>
