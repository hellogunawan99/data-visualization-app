...
// Function to convert date to UTC+8
const convertToUTC8 = (date) => {
  return new Date(date.getTime() + (8 * 60 * 60 * 1000));
};

useEffect(() => {
  const today = convertToUTC8(new Date());
...
const fetchData = async (start, end) => {
  ...
  setError('');
  try {
    const response = await fetch(`/api/displayProblemType?startDate=${start}&endDate=${end}`);
...
const fetchMTDData = async () => {
  try {
    const response = await fetch('/api/mtdDisplayProblemType');
    ...
    const data = await response.json();
    console.log('Fetched MTD data:', data);
...
const processData = (data, start, end) => {
  ...
  data.forEach(item => {
    const dateStr = formatDate(convertToUTC8(new Date(item.date)));
    
    if (groupedData[dateStr]) {
      if (!groupedData[dateStr][item.analysis]) {
        groupedData[dateStr][item.analysis] = 0;
      }
      groupedData[dateStr][item.analysis]++;
    }
  });

  const analysisTypes = [...new Set(data.map(item => item.analysis))];

  return {
    labels: allDates,
    datasets: analysisTypes.map(analysis => ({
      label: analysis,
      data: allDates.map(date => groupedData[date][analysis] || 0),
...
const processMTDData = (data) => {
  ...
  const sortedAnalysis = Object.entries(analysisCounts)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10);  // Get top 10 analysis types

  const colors = sortedAnalysis.map(() => getRandomColor());

  return {
    labels: sortedAnalysis.map(([analysis]) => analysis),
    datasets: [{
      data: sortedAnalysis.map(([, count]) => count),
...
const handleEndDateChange = (e) => {
  ...
  const getCurrentMonth = () => {
    ...
    return months[convertToUTC8(new Date()).getUTCMonth()];
  };

  return (
    <Layout>
      <div className="container mx-auto p-4">
        <h1 className="text-2xl font-bold mb-4">Type Display Problem Jigsaw</h1>
          ...
          <div className="flex flex-wrap -mx-2">
            <div className="w-full lg:w-2/3 px-2">
              <div className="mb-4 bg-white p-4 rounded-lg shadow" style={{ height: '500px' }}>
                <h2 className="text-xl font-semibold mb-2">Pekerjaan Display Problem</h2>
            ...
            <div className="w-full lg:w-1/3 px-2">
              <div className="mb-4 bg-white p-4 rounded-lg shadow">
                <h2 className="text-lg font-semibold mb-2">Top Pekerjaan Display Problem - {getCurrentMonth()}</h2>
